...
const Doctors = () => {
  ...
  return (
    <div>
      <p className='text-white-600'>Browse through the doctors specialist.</p>
      <div className=' text-white flex flex-col sm:flex-row items-start gap-5 mt-5'>
        <button onClick={() => setShowFilter(!showFilter)} className={`py-1 px-3 border rounded text-sm  transition-all sm:hidden ${showFilter ? 'bg-primary text-white' : ''}`}>Filters</button>
        <div className={`flex-col gap-4 text-sm text-white-600 ${showFilter ? 'flex' : 'hidden sm:flex'}`}>
